...
import { IProduct, IProductResponse, IProductsResponse, IOrderRequest, IOrderItemRequest, IOrderResponse, IOrder } from "./interfaces";
...
<h5>Vi har ${inStockItems().length} sorters godis i lager. ${outofStockItems().length
    } Sorter är slutsålda</h5>`;
};


const renderProductStatus = (product: IProduct) => {
  return product.stock_status === "instock" && product.stock_quantity && renderProductQuantity(product) ?
...
const renderProductDescription = (product: IProduct) => {
  ...
  document.querySelector("#product-description")?.querySelector(`#product-description-add`)?.addEventListener("click", (e) => {
    const target = e.target as HTMLElement;
...
const totalPrice = (products: IProduct[]) => {
  const calcPriceCart = products
    .map((product) => product.price * Number(localStorage.getItem(String(product.id))))
  ...
  const cartItems = (document.querySelector(".cart-render")!.innerHTML =
    productsInCart
      .map(
        ...
        <p>${Number(localStorage.getItem(String(product.id))) * product.price
          } kr</p>
      </div>
        ...
        <div class="test d-flex justify-content-end">
          <div class="counter-icon m-2">
            <button class="reduce-btn" data-product-id=${product.id
          }>-</button>
        <span class="sum-products">${localStorage.getItem(
            String(product.id)
          )} </span>

              <button class="increase-btn" data-product-id=${product.id
          }>+</button>
          </div>
        </div>
    <hr>
          ...
          <div class="card-body p-4">
            <div class="row d-flex justify-content-between align-items-center">
              <div class="col-md-2 col-lg-2 col-xl-2">
                <img class="card-img-top mx-auto d-block menu-img img-fluid" src="https://bortakvall.se${product.images.thumbnail}" alt="picture of ${product.name}"/>
              </div>
              <div class="col-md-3 col-lg-3 col-xl-3">
                <p class="lead fw-normal mb-2">${product.name}</p>
                <p><span class="text-muted"></span>${product.price}kr<span class="text-muted"></span></p>
              </div>
              <div class="col-md-3 col-lg-3 col-xl-2 d-flex">

              <span class="sum-products">Antal ${localStorage.getItem(String(product.id))} </span>
              </div>
...
const orderFormRequest = async () => {
  const firstNameInput = document.querySelector<HTMLInputElement>('#inputFirstName')!.value
  const lastNameInput = document.querySelector<HTMLInputElement>('#inputLastName')!.value
  const adressInput = document.querySelector<HTMLInputElement>('#inputAddress')!.value
  let zipInput = document.querySelector<HTMLInputElement>('#inputZip')!.value
  const cityInput = document.querySelector<HTMLInputElement>('#inputCity')!.value
  const emailInput = document.querySelector<HTMLInputElement>('#inputEmail')!.value
  let phoneInput = document.querySelector<HTMLInputElement>('#inputPhone')!.value
  ...
  // console.log(totalProductsTotalInCart);
...
const orderItemsRequest = () => {
  return productsAddedInCart().map((product) => <IOrderItemRequest>{
...
const orderSubmitForm = () => {
  ...
  document.querySelector<HTMLFormElement>('#form-customer')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    if (orderResponse.status === 'success') {
      ...
      Object.keys(orderResponse.data)
        .forEach(key => {
